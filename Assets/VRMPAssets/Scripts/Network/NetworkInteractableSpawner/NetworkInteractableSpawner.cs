...
namespace UnityEngine.XR.Templates.VRMultiplayer
{
    public class NetworkInteractableSpawner : NetworkBehaviour
    {
        ...
        {
            if (NetworkManager.Singleton != null && sessionOwnerId == NetworkManager.Singleton.LocalClientId)
            {
        ...
        void Update()
        {
            // Skip network operations if NetworkManager is not available (offline mode)
            if (NetworkManager.Singleton == null)
                return;
