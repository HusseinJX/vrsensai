...
#pragma warning restore CS0618 // Type or member is obsolete
        ...
        /// <summary>
        /// Safely updates the affordance state, catching any null reference exceptions
        /// that may occur with the deprecated BaseAffordanceStateProvider.
        /// </summary>
        /// <param name="stateData">The affordance state data to apply.</param>
        void SafeUpdateAffordanceState(AffordanceStateData stateData)
        {
            if (m_AffordanceStateProvider == null) return;
            
            try
            {
#pragma warning disable CS0618 // Type or member is obsolete
                m_AffordanceStateProvider.UpdateAffordanceState(stateData);
#pragma warning restore CS0618 // Type or member is obsolete
            }
            catch (System.NullReferenceException)
            {
                // Silently handle null reference exceptions from deprecated affordance system
                // This can happen when internal receivers are not properly initialized
            }
        }
...
#pragma warning restore CS0618 // Type or member is obsolete
        ...
        public virtual void OnHoverRpc(bool entered, ulong clientId)
        {
            if (clientId != NetworkManager.Singleton.LocalClientId)
            {
                ...
                SafeUpdateAffordanceState(new AffordanceStateData(Convert.ToByte(entered ? 2 : (isInteracting ? 4 : 0)), 1.0f));
            }
        }
        ...
        public virtual void OnActivateRpc(bool activate, ulong clientId)
        {
            ...
            {
                Activated(activate);
                SafeUpdateAffordanceState(new AffordanceStateData(Convert.ToByte(activate ? 5 : isInteracting ? 4 : 0), 1.0f));
            }
        }
        ...
        protected virtual void OnIsInteractingChanged(bool oldValue, bool newValue)
        {
            ...
            SafeUpdateAffordanceState(new AffordanceStateData(Convert.ToByte(newValue ? 4 : 0), 1.0f));
