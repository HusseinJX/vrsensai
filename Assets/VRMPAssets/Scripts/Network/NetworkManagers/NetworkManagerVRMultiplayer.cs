...
namespace XRMultiplayer
{
    ...
    public class NetworkManagerVRMultiplayer : NetworkManager
    {
        ...
        [SerializeField, Tooltip("Disable networking for local-only mode (better performance)")]
        bool m_DisableNetworking = false;

        ///<inheritdoc/>
        void Awake()
        {
            // Check if offline mode is enabled and disable network manager for better performance
            // Check both the instance and the serialized field for flexibility
            bool shouldDisable = m_DisableNetworking;
            if (!shouldDisable && XRINetworkGameManager.Instance != null)
            {
                shouldDisable = XRINetworkGameManager.Instance.offlineMode;
            }

            if (shouldDisable)
            {
                enabled = false;
                Utils.Log("[NetworkManager] Disabled for offline mode performance optimization.", 0);
                return;
            }
