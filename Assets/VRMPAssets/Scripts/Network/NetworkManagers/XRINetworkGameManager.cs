...
namespace XRMultiplayer
{
    ...
    public class XRINetworkGameManager : MonoBehaviour
    {
        ...
        /// <summary>
        /// If true, disables authentication and multiplayer services for a faster local-only experience.
        /// </summary>
        ...
        /// <summary>
        /// Action for when a session owner is promoted.
        /// </summary>
        ...
        protected virtual async void Awake()
        {
            ...
            if (offlineMode)
            {
                Utils.Log($"{k_DebugPrepend}Offline Mode Enabled. Skipping Lobby/Auth Managers.", 0);
                m_Connected.Value = false;
                m_ConnectionState.Value = ConnectionState.None;
                
                // We still need to find managers if we want to avoid null refs later or just disable them?
                // Best to just leave them be but not initialize auth.
                TryGetComponent(out m_SessionManager);
                TryGetComponent(out m_AuthenticationManager);
...
# endif
        ...
        /// <summary>
        /// Sets the initial player position to be forward in the room
        /// </summary>
        void SetInitialPlayerPosition()
        {
            ...
            if (xrOrigin != null)
            {
                // Preserve Y position, only change X and Z
                Vector3 currentPos = xrOrigin.transform.position;
                Vector3 forwardPosition = new Vector3(0, currentPos.y, 8.0f); // Slightly less forward
                xrOrigin.transform.position = forwardPosition;
                Debug.Log($"XRINetworkGameManager: Set initial XR Origin position to {forwardPosition} (preserved Y: {currentPos.y})");
            }
        }
        
        /// <summary>
        /// Ensures player position stays forward after all components initialize
        /// </summary>
        void EnsureForwardPosition()
        {
            StartCoroutine(DelayedPositionCheck());
        }
        
        System.Collections.IEnumerator DelayedPositionCheck()
        {
            ...
            {
                Vector3 currentPos = xrOrigin.transform.position;
                // If Z is less than 5, we're too far back, move forward
                if (currentPos.z < 5.0f)
                {
                    Vector3 forwardPosition = new Vector3(0, currentPos.y, 8.0f);
                    xrOrigin.transform.position = forwardPosition;
                    Debug.Log($"XRINetworkGameManager: Corrected XR Origin position back to forward: {forwardPosition}");
                }
            }
        }
        ...
        {
            // Set initial player position early
            SetInitialPlayerPosition();
            
            // Auto-initialize desktop mode if applicable
            gameObject.AddComponent<DesktopModeManager>();
            
            // Auto-setup VR Shooting
            gameObject.AddComponent<VRShooterSetup>();

            // Auto-setup Battery Game Manager
            gameObject.AddComponent<BatteryGameManager>();
            
            // Ensure position stays forward after all components initialize
            EnsureForwardPosition();

            if (offlineMode)
            {
                Utils.Log($"{k_DebugPrepend}Offline Mode: Skipping network initialization for better performance.");
                
                // Disable NetworkManager to save performance
                var networkManager = FindFirstObjectByType<NetworkManager>();
                if (networkManager != null)
                {
                    networkManager.enabled = false;
                    Utils.Log($"{k_DebugPrepend}NetworkManager disabled for offline mode.");
                }
            }
            else
            {
                // Only subscribe to network events if not in offline mode
                if (NetworkManager.Singleton != null)
                {
                    ...
                    Utils.Log($"{k_DebugPrepend}Starting Local Host.");
                    HostLocalConnection();
                }
            }
        }
        ...
        async void ShutDown()
        {
            ...
            // Skip session cleanup in offline mode for faster shutdown
            if (!offlineMode && m_SessionManager != null)
            {
